...
function Invoke-Preflight {
    ...
    Write-InstallerLog -Message 'Preflight passed. Paste launcher mode will be resolved from selected package.'

    return [pscustomobject]@{
        Ok = $true
        HasWindowsTerminal = [bool](Test-CommandExists -Name 'wt.exe')
    }
}
...
function Write-PasteWrapper {
    ...
    try {
        if (Test-Path -LiteralPath $WrapperPath) {
            $template = Get-Content -LiteralPath $WrapperPath -Raw
            if ($template -match 'ShellExecute\s+"pwsh\.exe"' -or $template -match 'Using elevated pwsh\.exe paste launcher') {
                $launcherMode = 'pwsh'
            }
            elseif ($template -match 'ShellExecute\s+"wt\.exe"' -or $template -match 'using Windows Terminal') {
                $launcherMode = 'wt'
            }
        }
    }
    ...
    Write-InstallerLog -Message ("Resolved paste launcher mode from package: {0}" -f $launcherMode)

    $rcpPath = Join-Path $InstallRoot 'rcp.ps1'
    if ($launcherMode -eq 'pwsh') {
        $content = @"
